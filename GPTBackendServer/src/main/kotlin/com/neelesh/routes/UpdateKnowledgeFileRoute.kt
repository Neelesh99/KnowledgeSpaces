...
data class SimpleKnowledgeFileUpdateRequest(
    val knowledgeFileId: String,
    val email: String,
    val newName: String?,
    val newBlobs: List<String>?
)

// the body lens here is imported as an extension function from the Jackson instance
val simpleKnowledgeFileUpdateRequest = Body.auto<SimpleKnowledgeFileUpdateRequest>().toLens()

object UpdateKnowledgeFileRoute {
    ...
    private val spec = "/knowledgeFile/update" meta {
        summary = "echoes the name and message sent to it"
        receiving(simpleKnowledgeFileUpdateRequest to SimpleKnowledgeFileUpdateRequest("knowledgeFileName", "someEmail", null, null))
        returning(Status.OK, simpleKnowledgeFileUpdateRequest to SimpleKnowledgeFileUpdateRequest("knowledgeFileName", "someEmail", null, null))
    ...
    private fun echo(knowledgeFileHandler: KnowledgeFileHandler): HttpHandler = { request: Request ->
        val received = simpleKnowledgeFileUpdateRequest(request)
        val result = knowledgeFileHandler.update(received)
        result.fold(
            {
