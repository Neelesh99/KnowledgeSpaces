...
export interface ComposeSetProps {
    files: KnowledgeFile[],
    setChosenComposeFile: (chosenFile: KnowledgeFile) => void
}

export default function ComposeSet(props: ComposeSetProps) {
    const [selected, setSelected] = useState(defaultFiles[0])
              ...
              />
                    ...
                    >
                        <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                            {props.files.map((person, personIdx) => (
                                <Listbox.Option
                                    key={personIdx}
                                    className={({ active }) =>
